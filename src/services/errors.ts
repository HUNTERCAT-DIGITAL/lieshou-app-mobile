...
/** 网络层错误（断网/DNS/TLS/超时）：无 status */
export function isNetworkError(e: unknown): boolean {
  return isApiError(e) && e.status === undefined;
}

/** 其他服务端错误（5xx / 4xx 非 404） */
export function isServerError(e: unknown): boolean {
  return isApiError(e) && e.status !== undefined && e.status !== 401 && e.status !== 404;
}

/** 取可展示的错误消息（后端 message 优先，网络错误给通用文案） */
export function getErrorMessage(e: unknown): string {
  if (isApiError(e)) {
    if (e.status === undefined) return "网络异常，请检查连接后重试";
    return e.message;
  }
  return "发生未知错误";
}
